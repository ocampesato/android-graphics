...
public class LineGraph2D extends Activity 
{
      ...
      private void renderLineSegments(Canvas canvas)
      {
       //for(int bar=0; bar<barCount; bar++) 
         for(int bar=0; bar<barCount-1; bar++) 
         {
            ...
            // render single bar 
            barPaint.setColor(barColors[bar%2]); 
            canvas.drawLine(currentX1+barWidth/2, 
                            currentY1,
                            currentX2+barWidth/2,
                            currentY2,
                            linePaint);

            // render left dot... 
            ovalRectF = new RectF(currentX1+barWidth/2-dotRadius/2,
                                  currentY1-dotRadius/2,
                                  currentX1+barWidth/2+dotRadius/2, 
                                  currentY1+dotRadius/2);

            canvas.drawOval(ovalRectF, dotPaint);
 
            // render right dot... 
            ovalRectF = new RectF(currentX2+barWidth/2-dotRadius/2,
                                  currentY2-dotRadius/2,
                                  currentX2+barWidth/2+dotRadius/2, 
                                  currentY2+dotRadius/2);
